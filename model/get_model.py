...
def get_model(cfg):
    ...
    if cfg['backbone']['type'] == 'VGG13_backbone':
        from model.backbone import VGG13_backbone
        backbone = VGG13_backbone(cfg['in_channels'])
    ...
    if cfg['head']['type'] == 'VGG13_head':
        from model.head import VGG13_head
        head = VGG13_head(cfg['in_features'], cfg['out_features'])
